...
class TestIsJetsonPlatform:
    """is_jetson_platform() reads /proc/device-tree/model on Linux/Jetson and
    must return False (not raise) on any other OS or when the file is unreadable."""

    def test_jetson_model_returns_true(self):
        from unittest.mock import patch, mock_open
        from rspy.pytest.device_helpers import is_jetson_platform
        with patch('builtins.open', mock_open(read_data='NVIDIA Jetson AGX Orin')):
            assert is_jetson_platform() is True

    def test_non_jetson_model_returns_false(self):
        from unittest.mock import patch, mock_open
        from rspy.pytest.device_helpers import is_jetson_platform
        with patch('builtins.open', mock_open(read_data='Raspberry Pi 5')):
            assert is_jetson_platform() is False
    ...
    def test_keyboard_interrupt_propagates(self):
        """Regression: bare `except:` used to swallow KeyboardInterrupt/SystemExit."""
        from unittest.mock import patch
        from rspy.pytest.device_helpers import is_jetson_platform
