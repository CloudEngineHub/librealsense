...
if test_minz_filter:
    with test.closure("Get MinZ embedded filter options"):
        ...
        check_option_in_list(rs.option.filter_magnitude, minz_options)
    ...
    with test.closure("MinZ embedded filter defaults"):
        ...
        test.check_equal(minz_embedded_filter.get_option(rs.option.filter_magnitude), MINZ_RATIO_DEFAULT_INDEX)
    ...
    with test.closure("MinZ embedded filter set/get options"):
        ...
        set_get_filter_option_value(minz_embedded_filter, rs.option.filter_magnitude, 2.0)
    ...
    with test.closure("MinZ invalid ratio rejected"):
        ...
        test.check_throws(
            lambda: minz_embedded_filter.set_option(rs.option.filter_magnitude, 3.0),
            RuntimeError)
